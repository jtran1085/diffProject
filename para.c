...
void para_printnor(para* p, void (*fp)(const char*), int type) {
  if (p == NULL) { return; }
  if(type == 0){
  printf("%dd,%d\n", p->start, p->stop);}//when printing left
  else if(type == 1){
  printf("%da,%d\n", p->start, p->stop);}//when printing right
  for (int i = p->start; i <= p->stop && i != p->filesize; ++i) { fp(p->base[i]); }
}

void para_printcon(para* p, void (*fp)(const char*), int type) {
  if (p == NULL) { return; }
  if(type == 0){
  printf("***%d,%d****\n", p->start, p->stop);}//when printing left
  else if(type == 1){
